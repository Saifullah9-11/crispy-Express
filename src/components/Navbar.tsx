              ...
              {user ? (
                <div className="flex items-center gap-4 pl-4 border-l border-white/10">
                  <div className="flex flex-col items-end">
                    <span className="text-[10px] font-black uppercase tracking-widest text-primary leading-none mb-1">Hello,</span>
                    <span className="text-xs font-black text-white uppercase tracking-tight truncate max-w-[100px]">{user.name.split(' ')[0]}!</span>
                  </div>
                ...
                <button onClick={() => setIsLoginModalOpen(true)} className="flex items-center gap-2 bg-white/5 hover:bg-white/10 px-5 py-3 rounded-xl transition-all group">
          ...
          {isOpen && (
            ...
            >
              <div className="px-4 py-8 flex flex-col gap-6 text-center">
                {user && (
                  <div className="mb-4">
                    <p className="text-primary font-black uppercase tracking-widest text-[10px] mb-1">Hello,</p>
                    <p className="text-white font-display font-black text-2xl uppercase italic">{user.name}!</p>
                  </div>
                )}
                {navLinks.map((link) => (
                  <Link
